...
export class Register2Component implements OnInit {
    @ViewChild('acceptTermsModal') private acceptTermsModal: ModalDialogComponent;
    ...
    ngOnInit(): void {
        ...
        this.titleService.setTitle('Register - Swarm IO');
    ...
    loginWithGooglePopup() {
        if (this.acceptTerms?.valid && (this.acceptTerms?.touched || this.acceptTerms?.dirty)) {
            // terms accepted, allow third party sign in
            this.auth.signInWithPopup(new firebase.auth.GoogleAuthProvider()).then(result => {
                this.router.navigate(['/console'])
            }).catch(err => console.log(err))
        } else {
            this.acceptTerms?.markAsDirty()
        }
    }
}
